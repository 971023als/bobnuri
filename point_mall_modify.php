<?php
    ...
    if ($level <= 1) {
        ...
        $sql = "UPDATE point_mall SET product_name='$p_name_esc', point='$point_esc'";
        if ($upfile_name) {
            $sql .= ", file_name='$upfile_name', file_type='$file_ext', file_copied='$copied_file_name'";
        }
        $sql .= " WHERE num=$num";
        ...
        // Level 4/5: Prepared
        if ($upfile_name) {
            $sql = "UPDATE point_mall SET product_name=?, point=?, file_name=?, file_type=?, file_copied=? WHERE num=?";
            $stmt = mysqli_prepare($con, $sql);
            mysqli_stmt_bind_param($stmt, "sssssi", $product_name, $point, $upfile_name, $file_ext, $copied_file_name, $num);
        } else {
            $sql = "UPDATE point_mall SET product_name=?, point=? WHERE num=?";
            $stmt = mysqli_prepare($con, $sql);
    ...
    echo "<script>location.href = 'point_mall_buy.php?num=$num';</script>";
?>
